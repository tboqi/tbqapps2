...
<dl>
<dt>我收藏的传闻</dt>
<#if myCollectionArticles?exists && myCollectionArticles.size() &gt; 0>
	<#list myCollectionArticles as map>
		<dd><span class="borderbottom"><a href="${base}/r/#{map.article.id}">${map.article.title}</a><span class="del_vote"><#if map.author?exists><a href="${base}/user/viewuser?memberId=#{map.author.id}">${map.author.userName}</a><#else>匿名</#if>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<#if member?exists && memberId=member.id><#if map.article.state =0 && map.article.channelId = 0>[<a href="#" onclick="return showStartDD('pubDD_${map_index}');">发起投票</a>]</#if>[<a href="${base}/user/removeCollection.action?articleId=#{map.article.id}">删除</a>]</#if></span></span></dd>
        ...
        <dd id="pubDD_${map_index}" style="display: none" >
        	<span class="borderbottom">
                <form action="${base}/addtochannel!add.action" method="post" onsubmit="return checkStartForm('pubchlId_${map_index}','pubrate_${map_index}','pubcost_${map_index}');">
                    ...
                    <input type="hidden" name="toPage" value="colllist" />
...
<#else>
<dd><span class="borderbottom">目前还没有收藏传闻</span></dd>
</#if>
</dl>
</div>
