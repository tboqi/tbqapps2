...
function screen(aid){
	$.get("/cc/clew.action?flag=screen&aid=" + aid, function(data){
  		//alert(data);
...
function unScreen(aid){
	$.get("/cc/clew.action?flag=unScreen&aid=" + aid, function(data){
  		//alert(data);
		//window.location.reload();
		document.getElementById("li" + aid).innerHTML=data;
	});
...
<table width="400" border="0" cellspacing="0" cellpadding="0">
		<tr>
				<td><form name="form1" method="post" action="/cc/clew.action">
				关键字：
...
<select name="property" id="property">
		...
		<option value="1" <#if proterty?exists><#if proterty=1>selected</#if></#if>>线索标题</option>
</select> 
...
<table border="1" cellspacing="0" cellpadding="0">
		...
		<ul>
			<li id="li#{article.id}">#{article.id}. <a href="/showRemark!showClew.action?remarkId=#{article.id}" target="_blank">${article.title}</a>&nbsp;
			<#if article.status=-1>
				[<font color="red">被屏蔽</font>]&nbsp;[<a href="#" onclick="unScreen(#{article.id})">取消屏蔽</a>]
			<#else>
				[正常]&nbsp;[<a href="#" onclick="screen(#{article.id})">屏蔽</a>]</#if>
			[<a href="/r/#{article.articleId}" target="_blank">原文</a>]
			</li>
		</ul>
		</#list>
		</td></tr>		
