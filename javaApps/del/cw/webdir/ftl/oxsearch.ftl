...
<div id="content">
  <form id="formsearch" name="formearch" action="${base}/ox/oxsearch.action?pn=1" method="post">
      <div id="search">
          ...
          <input class="inputtext01" type="text" name="qs" id="" /><input class="inputimage01" type="image" alt="请点击进行搜索" title="请点击进行搜索" src="${base}/images/btn_search.gif" />
      </div>
  </form>
  ...
  <dl>
    <dt class="blue"><a href="${base}/o/${item.id}">${item.title}</a></dt>
    <dd class="tag">作者：${item.author}&nbsp;<#assign labelList = map.labelList><#if labelList?exists && labelList.size() &gt; 0 ><span>标签：</span><#list labelList as label><a href="${base}/ox/oxsearch.action?qs=${stack.findValue("@com.cc.cw.util.URLCoderUtil@encode('${label}','UTF-8')")}&search=label&pn=1" title="${label}">${label}</a>&nbsp;&nbsp;<#if label_index=2><#break></#if></#list></#if>
	</dd>
    <dd>
      <p><#if item.content.length() &gt; 150>${item.content?substring(0, 150)}<#else>${item.content}</#if>.....</p>
    </dd>
  </dl>
  </#list>
  </#if>
...
<h1 class="title"><a href="/ox/">回到首页</a></h1>
</body>
</html>
