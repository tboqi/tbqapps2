		...
		if (! empty ( $this->children )) //If we've got some mime parts we need to stick them onto the end of the message
{
	...
	public function addChild(Swift_Message_Mime $mime, $id = null, $after = 1) {
		...
		$id = ( string ) $id;
		if ($after == - 1)
			$this->children = array_merge ( array ($id => $mime ), $this->children );
		else
			$this->children [$id] = $mime;
	...
	public function getChild($id) {
		if ($this->hasChild ( $id )) {
			...
			throw new Swift_Message_MimeException ( "Cannot retrieve child part identified by '" . $id . "' as it does not exist.  Consider using hasChild() to check." );
		}
	}
	/**
	...
	public function removeChild($id) {
		$id = ( string ) $id;
		if (! $this->hasChild ( $id )) {
			throw new Swift_Message_MimeException ( "Cannot remove child part identified by '" . $id . "' as it does not exist. Consider using hasChild() to check." );
		} else {
