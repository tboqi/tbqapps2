...
class Swift_Message_Headers {
	 ...
	 * @var array headers
	 */
	protected $emailContainingHeaders = array ("To", "From", "Reply-To", "Cc", "Bcc", "Return-Path", "Sender" );
	/**
	...
	public function set($name, $value) {
		...
		if (! is_array ( $value ) && $value !== null)
			$this->lowerHeaders [$lname] = ( string ) $value;
		else
			$this->lowerHeaders [$lname] = $value;
	}
	/**
	...
	public function setAttribute($header, $name, $value) {
		...
		if (! $this->has ( $header )) {
			throw new Swift_Message_MimeException ( "Cannot set attribute '" . $name . "' for header '" . $header . "' as the header does not exist. " . "Consider using Swift_Message_Headers-&gt;has() to check." );
		} else {
			Swift_ClassLoader::load ( "Swift_Message_Encoder" );
			if (! $this->getCharset () && Swift_Message_Encoder::instance ()->isUTF8 ( $value ))
				$this->setCharset ( "utf-8" );
			if (! isset ( $this->attributes [$lheader] ))
				$this->attributes [$lheader] = array ();
			if ($value !== null)
				$this->attributes [$lheader] [$name] = ( string ) $value;
			else
				$this->attributes [$lheader] [$name] = $value;
		}
	}
	/**
	...
	public function getAttribute($header, $name) {
		if (! $this->has ( $header )) {
			throw new Swift_Message_MimeException ( "Cannot locate attribute '" . $name . "' for header '" . $header . "' as the header does not exist. " . "Consider using Swift_Message_Headers-&gt;has() to check." );
		}
	...
	public function getEncoded($name) {
		...
		foreach ( $encoded_value as $key => $row ) {
			...
			if (Swift_Message_Encoder::instance ()->isHeaderSafe ( $row ) && ! $this->forceEncoding) {
				...
				$encoded_value [$key] = Swift_Message_Encoder::instance ()->header7BitEncode ( $row, 72, ($key > 0 ? 0 : (75 - (strlen ( $name ) + 5))), $this->LE );
			} elseif ($this->encoding == "Q") //QP encode required
{
				...
				$encoded_value [$key] = str_replace ( " ", "_", Swift_Message_Encoder::instance ()->QPEncode ( $row, (75 - (strlen ( $spec ) + 6)), ($key > 0 ? 0 : (75 - $used_length)), true, $this->LE ) );
			...
			} elseif ($this->encoding == "B") //Need to Base64 encode
{
				...
				$end = "?=";
				$used_length = strlen ( $name ) + 2 + strlen ( $spec ) + 2;
				$encoded_value [$key] = Swift_Message_Encoder::instance ()->base64Encode ( $row, (75 - (strlen ( $spec ) + 5)), ($key > 0 ? 0 : (76 - ($used_length + 3))), true, $this->LE );
			}
			...
			for($i = 0, $len = count ( $lines ); $i < $len; $i ++) {
				...
				if ($i > 0) {
					...
					if (substr ( $lines [$i], 0, 1 ) == '<' && substr ( $lines [$i], - 1 ) == '>')
						$lines [$i] = " " . $lines [$i];
					else
						$lines [$i] = " " . $spec . $lines [$i] . $end;
				} else {
					if (substr ( $lines [$i], 0, 1 ) != '<' || substr ( $lines [$i], - 1 ) != '>')
						$lines [$i] = $spec . $lines [$i] . $end;
				}
		...
		if (! empty ( $this->attributes [$lname] ))
			$this->cached [$lname] .= $this->buildAttributes ( $this->cached [$lname], $lname );
