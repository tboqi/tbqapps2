 ...
 *
 * $Id: Calendar_Event.php 3769 2008-12-15 00:48:56Z zombor $
 *
 * @package    Calendar
 * @author     Kohana Team
...
class Calendar_Event_Core extends Event_Observer {
	...
	// Boolean conditions
	protected $booleans = array ('current', 'weekend', 'first_day', 'last_day', 'last_occurrence', 'easter' );
	...
	public function notify($data) {
		...
		// Date conditionals
		$condition = array ('timestamp' => ( int ) $timestamp, 'day' => ( int ) date ( 'j', $timestamp ), 'week' => ( int ) $week, 'month' => ( int ) date ( 'n', $timestamp ), 'year' => ( int ) date ( 'Y', $timestamp ), 'day_of_week' => ( int ) date ( 'w', $timestamp ), 'current' => ( bool ) $current );
		...
		$this->caller->add_data ( array ('classes' => $this->classes, 'output' => $this->output ) );
	}
