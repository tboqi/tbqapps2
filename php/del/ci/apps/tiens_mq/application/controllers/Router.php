...
class Router extends CI_Controller {
    ...
    public function index() {
        //检查传入的参数
        $businessName = $this->input->post('businessName') ?: $this->input->get('businessName');
        ...
        $appKey = $this->input->post('appKey') ?: $this->input->get('appKey');
        if (empty($appKey)) {
        ...
        $syncFlag = $this->input->post('syncFlag') ?: $this->input->get('syncFlag'); //同步调用 0异步调用
        $syncFlag = intval($syncFlag);

        $messageJson = $this->input->post('messageJson') ?: $this->input->get('messageJson');
