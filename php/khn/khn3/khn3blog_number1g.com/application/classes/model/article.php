...
class Model_Article extends Model_Base {
	...
	function find($limit, $start, $fields='*') {
		...
		$sql = "select {$fields} from articles order by id desc limit :start, :limit";
	...
	function find_hot_articles() {
		...
		$sql = "select id,title from articles order by read_times desc limit 10";
		$query = DB::query(Database::SELECT, $sql);
		$results = $query->as_object()->execute();
	...
	function find_by_category_id($category_id, $limit, $start, $fields) {
		...
		$sql = "select {$fields} from articles where category_id=:category_id ".
...
// 		echo $sql;exit;
	...
	function find_by_tab_id($tab_id, $limit, $start, $fields) {
		$arr = array();
		
		$sql = "select {$fields} from articles " 
