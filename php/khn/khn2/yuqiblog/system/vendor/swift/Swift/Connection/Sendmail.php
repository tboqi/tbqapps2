...
class Swift_Connection_Sendmail extends Swift_ConnectionBase {
	...
	public function setPath($path) {
		if ($path == self::AUTO_DETECT)
			$path = $this->findSendmail ();
		$this->path = $path;
	}
	/**
	...
	public function findSendmail() {
		...
		if (! is_executable ( $path )) {
			...
			$common_locations = array ('/usr/bin/sendmail', '/usr/lib/sendmail', '/var/qmail/bin/sendmail', '/bin/sendmail', '/usr/sbin/sendmail', '/sbin/sendmail' );
	...
	public function setCommand($command) {
		if ($command == self::AUTO_DETECT)
			$command = $this->findSendmail () . " -bs";
	...
	protected function pipeIn($command, $end = "\r\n") {
		if (! $this->isAlive ())
			throw new Swift_ConnectionException ( "The sendmail process is not alive and cannot be written to." );
		if (! @fwrite ( $this->pipes [0], $command . $end ) && ! empty ( $command ))
			throw new Swift_ConnectionException ( "The sendmail process did not allow the command '" . $command . "' to be sent." );
		fflush ( $this->pipes [0] );
	}
	/**
	...
	protected function pipeOut() {
		if (strpos ( $this->getFlags (), "t" ) !== false)
			return;
		if (! $this->isAlive ())
			throw new Swift_ConnectionException ( "The sendmail process is not alive and cannot be read from." );
		$ret = "";
		$line = 0;
		while ( true ) {
			$line ++;
	...
	public function start() {
		...
		if (! @lstat ( $this->getPath () )) {
			throw new Swift_ConnectionException ( "Sendmail cannot be seen with lstat().  The command given [" . $this->getCommand () . "] does not appear to be valid." );
		}
		...
		if (! $this->isAlive ()) {
			throw new Swift_ConnectionException ( "The sendmail process failed to start.  Please verify that the path exists and PHP has permission to execute it." );
		}
	}
	/**
	...
	public function isAlive() {
		return ($this->proc !== false && is_resource ( $this->proc ) && is_resource ( $this->pipes [0] ) && is_resource ( $this->pipes [1] ) && $this->proc !== null);
	}
	/**
	 * Destructor.
