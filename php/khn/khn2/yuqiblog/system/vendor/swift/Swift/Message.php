...
class Swift_Message extends Swift_Message_Mime {
	 ...
	 * @var array
	 */
	protected $references = array ("parent" => array ("alternative" => null, "mixed" => null, "related" => null ), "alternative" => array (), "mixed" => array (), "related" => array () );
	...
	protected function setReference($where, $key, $ref) {
		if ($ref === $this)
			$this->references [$where] [$key] = false;
		else
			$this->references [$where] [$key] = $ref;
	}
	/**
	...
	protected function getReference($where, $key) {
		if (! $this->references [$where] [$key])
			return $this;
		else
			return $this->references [$where] [$key];
	}
	/**
	...
	public function generateId($idstring = null) {
		$midparams = array ("utctime" => gmstrftime ( "%Y%m%d%H%M%S" ), "pid" => getmypid (), "randint" => mt_rand (), "customstr" => (preg_match ( "/^(?<!\\.)[a-z0-9\\.]+(?!\\.)\$/iD", $idstring ) ? $idstring : "swift"), "hostname" => (isset ( $_SERVER ["SERVER_NAME"] ) ? $_SERVER ["SERVER_NAME"] : php_uname ( "n" )) );
		...
		if ($this->headers->has ( "Return-Path" )) {
			if (! $smtp)
				...
				if (strpos ( $path, ">" ) > strpos ( $path, "<" ))
					return substr ( $path, ($start = strpos ( $path, "<" )), ($start + strrpos ( $path, ">" ) + 1) );
				else
					return "<" . $path . ">";
			}
		}
	...
	public function setFrom($from) {
		if ($from instanceof Swift_Address)
			$from = $from->build ();
	...
	public function setTo($to) {
		if ($to) {
			...
			foreach ( $to as $key => $value ) {
				if ($value instanceof Swift_Address)
					$to [$key] = $value->build ();
			}
		}
	...
	public function setCharset($charset) {
		...
		if (($this->getEncoding () == "7bit") && (strtolower ( $charset ) == "utf-8" || strtolower ( $charset ) == "utf8"))
	...
	public function isFlowed() {
		if ($this->headers->hasAttribute ( "Content-Type", "format" ) && $this->headers->getAttribute ( "Content-Type", "format" ) == "flowed") {
			return true;
		} else
			return false;
	}
	...
	public function requestReadReceipt($request) {
		if ($request instanceof Swift_Address)
	...
	public function getPriority() {
		if ($this->headers->has ( "X-Priority" ))
			return $this->headers->get ( "X-Priority" );
		else
			return null;
	}
	/**
	...
	public function detach($id) {
		try {
			...
			throw new Swift_Message_MimeException ( "Something went wrong whilst trying to move some MIME parts during a detach(). " . "The MIME component threw an exception:<br />" . $e->getMessage () );
		}
	}
	/**
	...
	protected function fixContentType() {
		if (! empty ( $this->references ["mixed"] ))
			$this->setContentType ( "multipart/mixed" );
		elseif (! empty ( $this->references ["related"] ))
			$this->setContentType ( "multipart/related" );
		elseif (! empty ( $this->references ["alternative"] ))
	...
	protected function moveBranchIn($type, $nested_type, $from, $old_branch, $new_branch, $tag) {
		...
		switch ($new_branch) {
			case "related" :
				$this->setReference ( "related", $tag, $new ); //relatedRefs[$tag] = $new;
				break;
			case "mixed" :
				$this->setReference ( "mixed", $tag, $new ); //mixedRefs[$tag] = $new;
				break;
		}
		...
		switch (true) {
			case (! empty ( $this->references ["mixed"] ) && ! empty ( $this->references ["related"] ) && ! empty ( $this->references ["alternative"] )) :
				if (! isset ( $this->references ["related"] ["_alternative"] )) {
					$this->moveBranchIn ( "multipart/alternative", "multipart/alternative", $this->references ["alternative"], "alternative", "related", "_alternative" );
				}
				if (! isset ( $this->references ["mixed"] ["_related"] )) {
					$this->moveBranchIn ( "multipart/related", "multipart/alternative", $this->references ["related"], "related", "mixed", "_related" );
				}
				break;
			case (! empty ( $this->references ["mixed"] ) && ! empty ( $this->references ["related"] )) :
				if (! isset ( $this->references ["mixed"] ["_related"] )) {
					$this->moveBranchIn ( "multipart/related", "multipart/related", $this->references ["related"], "related", "mixed", "_related" );
				}
				break;
			case (! empty ( $this->references ["mixed"] ) && ! empty ( $this->references ["alternative"] )) :
				if (! isset ( $this->references ["mixed"] ["_alternative"] )) {
					$this->moveBranchIn ( "multipart/alternative", null, $this->references ["alternative"], "alternative", "mixed", "_alternative" );
				}
				break;
	...
	protected function moveBranchOut($from, $old_branch, $new_branch, $tag) {
		foreach ( $from as $id => $ref ) {
			if (! $ref)
				$ref = $this;
			$sign = (strtolower ( $ref->getContentType () ) == "text/html" || strtolower ( $ref->getContentType () ) == "multipart/alternative") ? - 1 : 1;
			$this->getReference ( "parent", $new_branch )->addChild ( $ref, $id, $sign );
			switch ($new_branch) {
				case "related" :
					$this->getReference ( "related", $tag )->removeChild ( $id );
					break;
				case "mixed" :
					$this->getReference ( "parent", $old_branch )->removeChild ( $id );
					break;
