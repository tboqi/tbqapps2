...
class Swift_Connection_SMTP extends Swift_ConnectionBase {
	...
	public function read() {
		if (! $this->handle)
			throw new Swift_ConnectionException ( "The SMTP connection is not alive and cannot be read from." . $this->smtpErrors () );
		$ret = "";
		$line = 0;
		while ( ! feof ( $this->handle ) ) {
			...
			if ($tmp === false && ! feof ( $this->handle )) {
				throw new Swift_ConnectionException ( "There was a problem reading line " . $line . " of an SMTP response. The response so far was:<br />[" . $ret . "].  It appears the connection has died without saying goodbye to us! Too many emails in one go perhaps?" . $this->smtpErrors () );
			}
			$ret .= trim ( $tmp ) . "\r\n";
			if ($tmp {3} == " ")
				break;
	...
	public function write($command, $end = "\r\n") {
		if (! $this->handle)
			throw new Swift_ConnectionException ( "The SMTP connection is not alive and cannot be written to." . $this->smtpErrors () );
	...
	public function runAuthenticators($user, $pass, Swift $swift) {
		...
		if ($tried == 0)
			throw new Swift_ConnectionException ( "No authentication mechanisms were tried since the server did not support any of the ones loaded. " . "Loaded authenticators: [" . implode ( ", ", array_keys ( $this->authenticators ) ) . "]" );
		else
