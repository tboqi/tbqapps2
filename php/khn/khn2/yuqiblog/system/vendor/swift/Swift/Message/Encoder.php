...
class Swift_Message_Encoder {
	...
	public function addressChunk($input) {
		...
		while ( preg_match ( '/^(.*?)(<' . self::CHEAP_ADDRESS_RE . '>)/s', $input, $matches ) ) {
			if (! empty ( $matches [1] ))
				$ret [($elements ++)] = $matches [1];
			$ret [('a' . ($elements ++))] = $matches [2];
			$input = substr ( $input, strlen ( $matches [0] ) );
		}
		if ($input != "")
			$ret [($elements ++)] = $input; //Whatever is left over
...
{
				if ($init_chunk && $init_chunk < (strlen ( $string ) + 2))
					$ret .= $le;
				$ret .= $le . $string;
			} else {
...
{
				if ($init_chunk && $init_chunk < (strlen ( $string ) + 3))
					$ret .= "=";
				$ret .= $le . $string;
			} else {
				...
				if ($init_chunk > 3) {
					if (preg_match ( '/^.{1,' . ($init_chunk - 5) . '}[^=]{2}(?!=[A-F0-9]{2})/', $string, $matches ) || preg_match ( '/^.{1,' . ($init_chunk - 6) . '}([^=]{0,3})?/', $string, $matches )) {
				...
				while ( preg_match ( '/^.{1,' . ($init_chunk - 5) . '}[^=]{2}(?!=[A-F0-9]{2})/', $string, $matches ) || preg_match ( '/^.{1,' . ($chunk - 6) . '}([^=]{0,3})?/', $string, $matches ) || (strlen ( $string ) > 0 && $matches = array ($string )) ) {
	...
	public function rawQPEncode($string, $bin = false) {
		...
		for($i = 0; $i < $len; $i ++) {
			...
			if ((! $bin && ($val == 32 || $val == 9 || $val == 10 || $val == 13)) || ($val >= 33 && $val <= 60) || ($val >= 62 && $val <= 126) && $val != 63) {
				$ret .= $string {$i};
			} else {
				$ret .= sprintf ( "=%02X", $val );
			}
	...
	public function QPEncodeFile(Swift_File $file, $chunk = 76, $le = "\r\n") {
		...
		while ( false !== $bytes = $file->readln () ) {
			...
			if (count ( $next [0] ))
				$cache->write ( "qp", $this->fixLE ( implode ( "=" . $le, $next [0] ), $le ) );
		}
