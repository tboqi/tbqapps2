...
if (! defined ( "SWIFT_TLS" ))
	define ( "SWIFT_TLS", Swift_Connection_SMTP::ENC_TLS );
if (! defined ( "SWIFT_SSL" ))
	define ( "SWIFT_SSL", Swift_Connection_SMTP::ENC_SSL );
if (! defined ( "SWIFT_OPEN" ))
	define ( "SWIFT_OPEN", Swift_Connection_SMTP::ENC_OFF );
if (! defined ( "SWIFT_SECURE_PORT" ))
	define ( "SWIFT_SECURE_PORT", Swift_Connection_SMTP::PORT_SECURE );
if (! defined ( "SWIFT_DEFAULT_PORT" ))
	define ( "SWIFT_DEFAULT_PORT", Swift_Connection_SMTP::PORT_DEFAULT );
...
class EasySwift {
	...
	public function useLogging($use = true) {
		...
		if ($use)
			...
			$log->setLogLevel ( Swift_Log::LOG_NOTHING );
	}
	/**
	...
	public function loadAuthenticator(Swift_Authenticator $auth) {
		if (method_exists ( $this->swift->connection, "attachAuthenticator" )) {
			...
			throw new Exception ( "SMTP authentication cannot be used with connection class '" . get_class ( $this->connection ) . "'. Swift_Connection_SMTP is needed" );
	}
	/**
