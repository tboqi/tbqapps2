		...
		if ($column === 'parent') {
			...
			if (empty ( $this->related [$column] )) {
				...
				if ($this->ORM_Tree_children === $this->table_name) {
					...
					$this->related [$column] = $model->where ( $this->ORM_Tree_parent_key, $this->object [$this->primary_key] )->find_all ();
				} else {
					// Find first selection of children
					$this->related [$column] = $model->where ( $this->foreign_key (), $this->object [$this->primary_key] )->where ( $this->ORM_Tree_parent_key, NULL )->find_all ();
				}
			}
