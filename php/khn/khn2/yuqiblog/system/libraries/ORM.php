 ...
 *
 * $Id: ORM.php 4050 2009-03-05 23:26:22Z jheathco $
 *
 * @package    ORM
 * @author     Kohana Team
...
class ORM_Core {
	...
	public function __get($column) {
		if (isset ( $this->ignored_columns [$column] )) {
			...
			return $this->related [$column] = $model->join ( $join_table, $join_col1, $join_col2 )->where ( $through->foreign_key ( $this->object_name, $join_table ), $this->object [$this->primary_key] )->find_all ();
		} elseif (in_array ( $column, $this->has_many )) {
			// one<>many relationship
			return $this->related [$column] = ORM::factory ( inflector::singular ( $column ) )->where ( $this->foreign_key ( $column, $column ), $this->object [$this->primary_key] )->find_all ();
			...
			if ($this->has ( $model, TRUE )) {
				...
				return $this->related [$column] = $model->in ( $model->table_name . '.' . $model->primary_key, $this->changed_relations [$column] )->find_all ();
			} else {
				// empty many<>many relationship
				return $this->related [$column] = $model->where ( $model->table_name . '.' . $model->primary_key, NULL )->find_all ();
			}
		} elseif (in_array ( $column, array ('object_name', 'object_plural', // Object
...
'has_one', 'belongs_to', 'has_many', 'has_and_belongs_to_many', 'load_with' )// Relationships
	...
	public function save() {
		if (! empty ( $this->changed )) {
			...
			if ($this->loaded === TRUE) {
				$query = $this->db->where ( $this->primary_key, $this->object [$this->primary_key] )->update ( $this->table_name, $data );
			...
			foreach ( $this->changed_relations as $column => $values ) {
				...
				if (! empty ( $added )) {
					foreach ( $added as $id ) {
						...
						$this->db->insert ( $join_table, array ($object_fk => $this->object [$this->primary_key], $related_fk => $id ) );
					}
				}
				
				if (! empty ( $removed )) {
					$this->db->where ( $object_fk, $this->object [$this->primary_key] )->in ( $related_fk, $removed )->delete ( $join_table );
				}
	...
	public function count_last_query() {
		if ($sql = $this->db->last_query ()) {
			...
			$result = $this->db->query ( 'SELECT COUNT(*) AS ' . $this->db->escape_column ( 'total_rows' ) . ' ' . 'FROM (' . trim ( $sql ) . ') AS ' . $this->db->escape_table ( 'counted_results' ) );
