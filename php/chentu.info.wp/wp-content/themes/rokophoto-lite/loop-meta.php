...
if ( is_home() ) : ?>
	...
	<?php elseif ( is_single() ) : ?>
		...
		<?php if ( has_category() ) : ?>
			...
			<h6>
				...
				printf( __( 'Home / <a href="%2$s">%1$s</a> / %3$s', 'rokophotolite' ), $category[0]->name, get_category_link( $category[0]->term_id ), get_the_title() );
				?>
			</h6>
		...
		<?php elseif ( 'portfolio' == get_post_type() ) : ?>
			...
			<h6>
				...
				printf( __( 'Home / Portfolio / %1$s', 'rokophotolite' ), get_the_title() );
				?>
			</h6>
		...
		<?php else : ?>
			...
			<h6>
				<?php
				/* translators: %1$s is the title */
				printf( __( 'Home / %1$s', 'rokophotolite' ), get_the_title() );
				?>
			</h6>
	...
	<?php elseif ( is_author() ) : ?>
		...
		<h6>
			...
			printf( __( 'Posts By: %1$s', 'rokophotolite' ), get_the_author_meta( 'display_name', $user_id ) );
			?>
		</h6>
