...
class Controller_Product extends Controller_Template {
    ...
    private $fields_for_main_content = '*';
    ...
    public function action_cate() {
        $product_model = new Model_Article();
        ...
        //分页
        $pagination = Pagination::factory(array(
            'total_items' => $product_model->count_by_category_id($category_id),
            'items_per_page' => $limit,
        )
        );
        $pagination->route_params(array('controller' => $this->request->controller(),
        ...
        $content->items = $product_model->find_by_category_id($category_id,
            $limit, $start, $this->fields_for_main_content);
        var_dump($content->items);exit;

        $content->pagination = $pagination;

        $this->template->content = $content;

        $model_category = new Model_Product_Category();
        $category = $model_category->get($category_id, 'name');
        $this->sub_title = "分类 {$category->name} 下的产品列表";
