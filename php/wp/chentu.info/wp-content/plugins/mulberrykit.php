...
Plugin URI: http://yan.me/dia/wordpress/kit
...
function get_recent_trackbacks($no_comments = 5, $before = '<li> ', $after = '</li>', $show_pass_post = false) {
	...
	global $wpdb;
	$request = "SELECT ID, comment_ID, comment_content, comment_author FROM $wpdb->posts, $wpdb->comments WHERE $wpdb->posts.ID=$wpdb->comments.comment_post_ID AND (post_status = 'publish' OR post_status = 'static') AND (comment_type = 'trackback' OR comment_type ='pingback')";
...
function get_mostcommented($limit = 5) {
    ...
    foreach ($mostcommenteds as $post) {
    	$post_id = empty($post->post_id) ? 0 : (int) $post->post_id;
...
function get_commentmembersstats($threshhold = 5) {
	...
	$comments = $wpdb->get_results("SELECT comment_author, comment_author_url, COUNT(comment_ID) AS 'comment_total' FROM $wpdb->comments WHERE comment_approved = '1' AND (comment_author != '桑葚') AND (comment_author != '')GROUP BY comment_author ORDER BY comment_total DESC");
	$no = 1;
