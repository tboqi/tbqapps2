<table>
	...
	<tr>
		<td><a href="<?php echo site_url("download/downloadsOfCategory/{$download->categoryID}"); ?>"><?php echo $download->categoryName; ?></a></td>
	</tr>
	<?php if($download->keywords && is_array($download->keywords)) { ?>
	<tr>
		<td>
			<?php foreach ($download->keywords as $keyword) { ?>
			<a href="<?php echo site_url("download/downloadsOfkeyword/{$keyword->id}"); ?>"><?php echo $keyword->keyword; ?></a>
			<?php } ?>
		</td>
	</tr>
	<?php } ?>
	...
	<tr>
    	...
    	<a href="<?php echo site_url("download/edit/{$download->id}") ?>">编辑</a>
    	<a href="<?php echo site_url("download/delete/{$download->id}") ?>">删除</a>
    	<?php } ?>
		</td>
	</tr>
</table>